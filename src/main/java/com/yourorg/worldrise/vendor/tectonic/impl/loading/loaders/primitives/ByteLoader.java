package com.yourorg.worldrise.vendor.tectonic.impl.loading.loaders.primitives;

import com.yourorg.worldrise.vendor.tectonic.api.depth.DepthTracker;
import com.yourorg.worldrise.vendor.tectonic.api.exception.LoadException;
import com.yourorg.worldrise.vendor.tectonic.api.loader.ConfigLoader;
import com.yourorg.worldrise.vendor.tectonic.api.loader.type.TypeLoader;
import org.jetbrains.annotations.NotNull;

import java.lang.reflect.AnnotatedType;


public class ByteLoader implements TypeLoader<Byte> {
    @Override
    public Byte load(@NotNull AnnotatedType t, @NotNull Object c, @NotNull ConfigLoader loader, DepthTracker depthTracker) {
        try {
            return ((Number) c).byteValue();
        } catch(ClassCastException e) {
            throw new LoadException("Data provided is not a byte. Data is type: " + c.getClass().getSimpleName(), e, depthTracker);
        }
    }
}
